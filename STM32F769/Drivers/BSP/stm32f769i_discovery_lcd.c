...
/** @defgroup STM32F769I_DISCOVERY_LCD_Exported_Variables STM32F769I DISCOVERY LCD Exported Variables
  * @{
  */
SECTION_DTCMRAM DMA2D_HandleTypeDef hdma2d_discovery;
SECTION_DTCMRAM LTDC_HandleTypeDef  hltdc_discovery;
SECTION_DTCMRAM DSI_HandleTypeDef hdsi_discovery;
...
uint8_t BSP_LCD_Init(void)
{
  ...
  fbaddr = (uint32_t) &doom_fb[640*400];
  osMessageQueuePut(fbqId, &fbaddr, 0, 0);
  return val;
}
...
void HAL_LTDC_LineEventCallback(LTDC_HandleTypeDef *hltdc)
{
  ...
  {
    BSP_LCD_SetLayerAddress(0, (uint32_t)newfb);
    fbaddr = (newfb == doom_fb)? &doom_fb[640*400] : doom_fb;
    val = (uint32_t)fbaddr;
    newfb = NULL;
    osMessageQueuePut(fbqId, &val, 0, 0);
  }
  HAL_LTDC_ProgramLineEvent(hltdc, 440);
}
...
/************************LTDC Initialization***********************************/  
  ...
  HAL_LTDC_ConfigColorKeying(&hltdc_discovery, 0xff << 8, 1);
  HAL_LTDC_EnableColorKeying(&hltdc_discovery, 1);

  HAL_LTDC_ProgramLineEvent(&hltdc_discovery, 440);
...
void BSP_LCD_LayerDefaultInit(uint16_t LayerIndex, uint32_t FB_Address)
{
  ...
  if (LayerIndex == 0)
  {
    ...
    Layercfg.WindowX1 = 80 + 640;
    Layercfg.WindowY0 = 40;
    Layercfg.WindowY1 = 40 + 400;
    Layercfg.PixelFormat = LTDC_PIXEL_FORMAT_L8;
    Layercfg.FBStartAdress = (uint32_t) doom_fb;
    Layercfg.Alpha = 255;
    Layercfg.Alpha0 = 0;
  ...
  else
  {
    ...
    Layercfg.FBStartAdress = (uint32_t) lvgl_fb;
    Layercfg.Alpha = 255;
    Layercfg.Alpha0 = 0;
    Layercfg.Backcolor.Blue = 0;
    Layercfg.Backcolor.Green = 0;
