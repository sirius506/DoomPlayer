...
debug_printf("CH %d, BBLERR\n", chnum);
    (void)USB_HC_Halt(hhcd->Instance, chnum);
  }
  else if (__HAL_HCD_GET_CH_FLAG(hhcd, chnum, USB_OTG_HCINT_STALL))
  {
debug_printf("CH %d, STALL\n", chnum);
...
debug_printf("CH %d, TXERR\n", chnum);
  ...
  else if (__HAL_HCD_GET_CH_FLAG(hhcd, chnum, USB_OTG_HCINT_XFRC))
  {
    ...
    if (hhcd->Init.dma_enable == 1U)
    {
      ...
      v2 = (v1 / hhcd->hc[chnum].max_packet) & 1U;
      v3 = (hhcd->hc[chnum].XferSize / hhcd->hc[chnum].max_packet) & 1U;
      if (v2 != v3)
      {
debug_printf("xfer_count = %d, XferSize = %d, max_packet = %d\n",
 ...
 hhcd->hc[chnum].max_packet);
  ...
  else if (__HAL_HCD_GET_CH_FLAG(hhcd, chnum, USB_OTG_HCINT_CHH))
  {
...
#ifdef ORG_CODE
#else
    else if ((hhcd->hc[chnum].state == HC_IDLE) && (hhcd->Init.dma_enable != 0))
    {
      hhcd->hc[chnum].state = HC_HALTED;
      hhcd->hc[chnum].urb_state = URB_DONE;
    }
    ...
    else
    {
      ...
      {
debug_printf("CHH @ HC_HALTED\n");
        return;
      }
    debug_printf("CHH: ch = %d, state = %d\n", chnum, hhcd->hc[chnum].state);
    }
