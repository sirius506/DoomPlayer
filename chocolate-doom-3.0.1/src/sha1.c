...
{
  osSemaphoreRelease(hashsemId);
}

void SHA1_ComputeDigest(uint8_t *digest, uint8_t *data, int data_len)
{
    if (hashsemId == 0)
      hashsemId = osSemaphoreNew(1, 0, &attributes_hashsem);
  HAL_HASH_SHA1_Start_IT(&hhash, data, data_len, digest);
  osSemaphoreAcquire(hashsemId, osWaitForever);
}
#endif
