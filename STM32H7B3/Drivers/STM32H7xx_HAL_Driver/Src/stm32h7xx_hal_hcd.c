...
static void HCD_HC_IN_IRQHandler(HCD_HandleTypeDef *hhcd, uint8_t chnum)
{
  ...
  {
debug_printf("HCINT_BBERR %d, %x, %x\n", ch_num, USBx_HC(ch_num)->HCINT, USBx_HC((uint32_t)ch_num)->HCINTMSK );
    __HAL_HCD_CLEAR_HC_INT(ch_num, USB_OTG_HCINT_BBERR);
#if 0
...
debug_printf("HC_INT_STALL @ %d\n", ch_num);
  ...
  {
debug_printf("HCINT_DTERR %d, %x, %x\n", ch_num, USBx_HC(ch_num)->HCINT, USBx_HC((uint32_t)ch_num)->HCINTMSK );
  ...
  {
debug_printf("HCINT_TXERR %d, %x, %x\n", ch_num, USBx_HC(ch_num)->HCINT, USBx_HC((uint32_t)ch_num)->HCINTMSK );
    __HAL_HCD_CLEAR_HC_INT(ch_num, USB_OTG_HCINT_TXERR);
#if 0
    hhcd->hc[ch_num].state = HC_XACTERR;
    (void)USB_HC_Halt(hhcd->Instance, (uint8_t)ch_num);
...
debug_printf("URB_ERROR [%d, %x] @ %d\n", ch_num, hhcd->hc[chnum].state, __LINE__);
      }
      else
      {
        hhcd->hc[ch_num].urb_state = URB_NOTREADY;
debug_printf("URB_NOTREADY [%d, %x] @ %d\n", ch_num, hhcd->hc[chnum].state, __LINE__);
    ...
    {
      hhcd->hc[ch_num].urb_state  = URB_NOTREADY;
//debug_printf("URB_NOTREADY (NAK) [%d, %x] @ %d\n", ch_num, hhcd->hc[chnum].state, __LINE__);
    ...
    else if (hhcd->hc[ch_num].state == HC_BBLERR)
    {
      hhcd->hc[ch_num].ErrCnt++;
      hhcd->hc[ch_num].urb_state = URB_ERROR;
debug_printf("URB_ERROR (BBLERR) [%d, %x] @ %d\n", ch_num, hhcd->hc[chnum].state, __LINE__);
    }
    else
    {
      /* ... */
